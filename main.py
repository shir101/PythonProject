...
#the function compares the new genome to the basc covid19 and saves the differences
def find_differntial(kmers_list, basic, families):
    for lineage in range(1,len(families)):  #iterating over he lineages (pages) - rows of the "families" array
        for position in range(0,len(families[lineage])): #iterating over the kmers in the lineage - cols of the "families" array
            if(lineage[position]!=basic[position]):
                families.insert(lineage[position])  #!!!!check if inserts into the next page end of kmers_list
...
if __name__ == '__main__':
    ...
    for filename in os.scandir(directory):
        if filename.is_file() and filename.path!='fasta\EPI_ISL_402124.fasta':
            ...
            sequence = extractFasta(path_to_file)
            kmers_list = kmers(4,4,sequence)
            #saving the differences between the new genome and the basic genome - !!compare new genome to the closest one in the kineage that is already in the databse
            kmers_list = find_differntial(kmers_list,families[0], families)
            print(kmers_list)
            #print(type(kmers_list[7]))
            families.append(kmers_list)
